	...
	async getAllFoldersOfPrivateDrive(driveId: DriveID, driveKey: DriveKey): Promise<ArFSPrivateFolder[]> {
		...
		while (hasNextPage) {
			...
			const folders: Promise<ArFSPrivateFolder>[] = edges.map(async (edge: GQLEdgeInterface) => {
				cursor = edge.cursor;
				const { node } = edge;
				const folderBuilder = await ArFSPrivateFolderBuilder.fromArweaveNode(node, this.arweave);
				return await folderBuilder.build(driveKey, node);
			});
			allFolders.push(...(await Promise.all(folders)));
		}
	...
	async getAllPrivateChildrenFilesFromFolderIDs(
		...
		while (hasNextPage) {
			...
			const files: Promise<ArFSPrivateFile>[] = edges.map(async (edge: GQLEdgeInterface) => {
				const { node } = edge;
				cursor = edge.cursor;
				const fileBuilder = await ArFSPrivateFileBuilder.fromArweaveNode(node, this.arweave);
				return await fileBuilder.build(driveKey, node);
			});
			allFiles.push(...(await Promise.all(files)));
		}
		return allFiles;
